...
- Под капотом используется [RocksDB](https://rocksdb.org/) — очень [эффективная БД](https://github.com/facebook/rocksdb) с поддержкой итераторов и эффективной работой на SSD.

- Основное назначение LittleVec — быть эффективной по памяти векторной БД. Низкий расход ОЗУ: 15-20 Мб даже при хранении миллионов векторов и гигабайтах реально хранящихся данных.
...
- В каждом запросе явно указывается имя базы данных. Поэтому можно использовать несколько баз одновременно.

- Лимит на размер payload ограничен возможностями RocksDB и параметром `max_body_size` RocksServer.
  
- В каждом поисковом запросе можно указать функцию расстояния вручную. Кроме того, метрику расстояния по умолчанию можно менять, обновив параметры базы данных.
